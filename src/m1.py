...
class CircleChanger(object):
    ...
    def swell_or_shrink_once(self, amount_to_swell_or_shrink):
        ...
        ################################################################
        if not (self.circle.radius + amount_to_swell_or_shrink < 1 ):
    ...
    def swallow(self, other_circle_changer):
        ...
        #   NO CREDIT if you use the distance formula here.
        ################################################################
        newCircle = rg.Circle(self.circle.center.halfway_to(other_circle_changer.circle.center),
                              0.5 * self.circle.center.get_distance_from(other_circle_changer.circle.center))
        newChanger = CircleChanger(newCircle.center.x, newCircle.center.y, newCircle.radius,
    ...
    def change_to_original_color(self):
        ...
        #   (below).  Third, implement and test this method.
        ################################################################
        self.circle.fill_color = self.initFillColor
    ...
    def change_to_next_color_in_tuple(self):
        ...
        ################################################################
        self.change_color(self.colorsChanged % len(self.colors))
        self.colorsChanged += 1
